...
import { Coords } from "./utils/mod.ts";
...
export function ofKing(chess: Chess, from: number, asColor?: PieceColor) {
  ...
  const kingPos = asColor === PieceColor.WHITE
    ? Boards.WHITE_KING_DEFAULT_POS
    : Boards.BLACK_KING_DEFAULT_POS;
...
function alignAxisMove(
  ...
  const moves: Move[] = [];
  const { up, down, left, right } = Boards.numToEdge[from];
  ...
  for (const line of lines) {
    const to = Coords.getCoords(from);
    while (line.steps > 0) {
...
function diagonalMove(
  ...
  const moves: Move[] = [];
  const { upRight, downRight, downLeft, upLeft } = Boards.numToEdge[from];
  const lines: MoveLine[] = [
    {
      dir: { x: 1, y: -1 },
      steps: upRight,
...
//   const board = Boards.copy(realBoard);
// }

export function generateMoves(chess: Chess) {
  const moves: Move[][] = [];
  const kingIndex = Boards.getKingIndex(chess.board, chess.current);

  let checkMate = true;
  let staleMate = false;

  // Generate moves
  for (let offset = 0; offset < Boards.BOARD_SIZE; offset++) {
...
export function isLegal(board: Board, testMove: Move, kingIndex: number) {
  ...
  const boardCopy = Boards.copy(board);
...
export function attacked(
  ...
  const offset = (opponentColor) === PieceColor.BLACK
    ? Direction.UP
    : Direction.DOWN;
  if (from + offset < 0 && from + offset >= Boards.BOARD_SIZE) {
    return false;
  }
  if (from % 8 != 0) {
